...
class GRAI96Test(unittest.TestCase):
    ...
    def test_ParseEPCTagUri(self):
        ...
        tagUri = "urn:epc:tag:grai-96:%s.%s.%s.%s" % (str(self._filter),str(self._companyPrefix),self._assetType,self._serialNumber)
        factory = EPCFactory()
    ...
    def test_ChangeSerialNumber(self):
        ...
        epc = self._grai96.encode(self._companyPrefix, None,self._assetType ,self._filter, self._serialNumber)
        print ("Serial Number Was %s" % epc.getSerialNumber())
        hex = epc.toHex()
        print ("Old Hex = %s" % hex) 
        ...
        factory = EPCFactory()
        epc = factory.parse(hex)
        self.assertEquals(int(epc.getSerialNumber()),newSerial)
    ...
    def test_Formats(self):
        ...
        #Start with this GRAI-96 20358468023395
        epc = self._grai96.encode(self._companyPrefix, None,self._assetType ,self._filter, self._serialNumber)
        #Turn the tagpy number into a hex value and parse it through the factory
        hex = epc.toHex()
        factory = EPCFactory()
    ...
    def _checkFields(self,epc):
        ...
        partitionValue = partitions.getPartitionValue(len(self._companyPrefix), "GRAI")
        
        self.assertEqual(epc.getFieldValue("Header"),"51")
        ...
        self.assertEqual(epc.getFieldValue("AssetType"),str(self._assetType))
        self.assertEqual(int(epc.getFieldValue("Serial")),int(self._serialNumber))
