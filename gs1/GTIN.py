...
class GTIN(GS1Number):
	'''Represents a GS1 GTIN
	  A GTIN (Global Trade Item Number) provides the global supply chain a solution for the identification 
	  of any item that is traded (priced, ordered, invoiced).
	  
	   GTINs may be 8, 12, 13 or 14-digits in length. Their data structures require up to 14-
	   digit fields, and *all* GTIN processing software should allow for 14 digits. 
	  
	'''
	def __init__(self, companyPrefix=None):
...
# 					#get the gtin14 body
# 					self._gtin = self._gs1[match.start()+4:match.end()+14]
...
# 				self._applicationIdentifiersList.append(self._gs1[match.start():match.end()])
# 				#get the gtin14 body
# 				self._gtin = self._gs1[match.start() + 2:16]
...
# 				return "(01)%s(21)%s" % (self._gtin,str(self._serialNumber).zfill(self.getFixedSerialNumberLength()))
# 			else:
# 				return "(01)%s(21)%s" % (self._gtin,str(self._serialNumber))
# 				
# 					
# 		else:
# 			if self.getUseFixedSerialNumber() == True:
# 				return "01%s21%s" % (self._gtin,str(self._serialNumber).zfill(self.getFixedSerialNumberLength()))
# 			else:
# 				return "01%s21%s" % (self._gtin, int(self._serialNumber))
# 					
# 	def toURN(self):
